...
or the [vault-ssh-helper](https://github.com/hashicorp/vault-ssh-helper) you can
use the available [Vagrantfile](https://www.vagrantup.com/) by running
`vagrant up`.
...
[hvault_inventory.py](./hvault_inventory.py) is a Python script that basically
uses the [HashiCorp Vault API client](https://github.com/hvac/hvac) and [PycURL](http://pycurl.io/)
...
list of managed hosts, then uses the IP Addresses to write
`/ssh/creds/otp_key_role` and retrive the created OTP credentials.

## Vault and host onfiguration

We will use Vagrant and the configured virtul machines, this will create three
...
```sh
$ vault server -dev --dev-listen-address=192.168.56.40:8200 &> /tmp/vault.log &
...
The [vault_ssh_helper_installation.sh](./vagrant_scripts/vault_ssh_helper_installation.sh)
...
ok: [server01] => {
    "msg": "2021/12/02 23:28:55 [INFO] vagrant@192.168.56.41 authenticated!"
}
ok: [server02] => {
    "msg": "2021/12/02 23:28:56 [INFO] vagrant@192.168.56.42 authenticated!"
}

TASK [print keyboard-interactive] *****************************************************************************************************
ok: [server01] => {
    "msg": "Dec  2 23:28:55 ubuntu-focal sshd[27451]: Accepted keyboard-interactive/pam for vagrant from 192.168.56.40 port 47962 ssh2"
}
ok: [server02] => {
    "msg": "Dec  2 23:28:56 ubuntu-focal sshd[27017]: Accepted keyboard-interactive/pam for vagrant from 192.168.56.40 port 52980 ssh2"
}

PLAY RECAP ****************************************************************************************************************************
server01                   : ok=5    changed=0    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
server02                   : ok=5    changed=0    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
```
